  ...
  $.fn.material_select = function (callback) {
    $(this).each(function(){
      ...
      $newSelect.on({
        ...
        'click': function (e){
          e.stopPropagation();
            if($(this).index($('.select-dropdown'))==-1)window.selectindex++;
          var selecttop = $('ul.multiple-select-dropdown').eq(0).css('top');
            var topsize = parseInt(selecttop.slice(0,-2));
            topsize=topsize<0?topsize-40:topsize+40
            $('ul.multiple-select-dropdown').eq(0).css({'top':topsize+'px'});
             if(!(window.selectindex%2)) multiple && (optionsHover || $newSelect.trigger('close'));
        }
      });
